...
bool GpuBackend::setup(int p_width, int p_height) {
  ...
  if (rd == nullptr) {
    UtilityFunctions::print("[gpu] no RenderingDevice (headless needs a real driver; run windowed)");
    return false;
  }
  ...
  if (!gpu_textures[0].is_valid() || !gpu_textures[1].is_valid()) {
    UtilityFunctions::print("[gpu] texture_create failed");
    return false;
  }

  //load and compile shader glsl
  if (!create_compute_pipeline()) {
    UtilityFunctions::print("[gpu] create_compute_pipeline failed");
    return false;
  }
  ...
  if (!uniform_sets[0].is_valid() || !uniform_sets[1].is_valid()) {
    UtilityFunctions::print("[gpu] uniform_set_create failed");
    return false;
  }
...
RID GpuBackend::create_gpu_texture() {
...
void GpuBackend::initialize_gpu_texture(RID texture_rid, const std::vector<uint8_t> &cells) {
...
void GpuBackend::load(const std::vector<uint8_t> &cells) {
...
bool GpuBackend::create_compute_pipeline() {
  ...
  if (compile_err != "") {
    UtilityFunctions::print("[gpu] shader compile error: ", compile_err);
    return false;
  }
...
RID GpuBackend::create_uniform_set(RID input_texture, RID output_texture) {
  //input texture
...
void GpuBackend::read_back(std::vector<uint8_t> &out) {
  if (rd == nullptr) {
    return;
  }
...
Ref<Texture2D> GpuBackend::get_display_texture() {
